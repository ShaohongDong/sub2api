<template>
  <div class="flex w-full flex-wrap items-center gap-2">
    <SearchInput
      ...
      class="basis-full md:max-w-[360px] md:min-w-[240px] md:flex-[1_1_280px]"
      @update:model-value="$emit('update:searchQuery', $event)"
      @search="$emit('change')"
    />
    <Select :model-value="filters.platform" class="min-w-[140px] flex-1 sm:flex-none sm:w-32 xl:w-36" :options="pOpts" @update:model-value="updatePlatform" @change="$emit('change')" />
    <Select :model-value="filters.type" class="min-w-[140px] flex-1 sm:flex-none sm:w-32 xl:w-36" :options="tOpts" @update:model-value="updateType" @change="$emit('change')" />
    <Select :model-value="filters.status" class="min-w-[140px] flex-1 sm:flex-none sm:w-32 xl:w-36" :options="sOpts" @update:model-value="updateStatus" @change="$emit('change')" />
    <Select :model-value="filters.group" class="min-w-[140px] flex-1 sm:flex-none sm:w-32 xl:w-36" :options="gOpts" @update:model-value="updateGroup" @change="$emit('change')" />
  </div>
</template>
